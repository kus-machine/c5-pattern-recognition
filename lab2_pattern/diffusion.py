...
def get_probs(image, mean_0, mean_1, cov_0, cov_1):
    ...
    PROB = np.zeros((image_resh.shape[0], 2), np.float64)
...
def fill_graph_edges(probs, epsilon=0.25):
    g_for_im = np.zeros((2 * probs.shape[0] - 1, 2 * probs.shape[1] - 1, 4), np.float64)
...
def diffusion_iter(graph_edges):
    new_graph_edges = np.zeros_like(graph_edges, np.float64)
...
@njit(fastmath=True)
def deletion_algorithm(g, epsilon):
...
def labeling(gr):
    ...
    gr = deletion_algorithm(gr, epsilon_max/4)
