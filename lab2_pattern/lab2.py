...
def fill_graph_edges(probs, epsilon=0.25):
    g_for_im = np.zeros((2 * probs.shape[0] - 1, 2 * probs.shape[1] - 1, 4), np.double)
...
def diffusion_iter(graph_edges_for_diffusion_iter):
    graph_edges = graph_edges_for_diffusion_iter.copy()
    new_graph_edges = np.zeros_like(graph_edges, np.double)
    for i in prange(graph_edges.shape[0]):
...
def deletion_alg(gr, epsilon):
    ...
    while True:
        ...
        if np.all(new_gr == gr1):
            print("Deletions algorithm has ended")
            break
        else:
            gr1 = new_gr.copy()
    return new_gr
